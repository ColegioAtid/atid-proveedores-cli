...
export default {
  ...
  methods: {
    ...
    ...mapActions("proveedores", ["sendFilesproveedor", "sendDataProveedor","getDataproveedor"]),
    ...
    async sendForm() {
      ...
      try {
        await this.sendFilesproveedor({files:this.files, tipo:'POST'});
        ...
        this.$router.push({name:'datos-generales-proveedores'});
      } catch (error) {
        // Error
        this.setShowErrorOrSuccessAlert({
